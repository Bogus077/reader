...
export const StudentLibrary: FC = () => {
  ...
  const categories = useMemo(() => {
    ...
    books.forEach(b => { if (b.category) s.add(b.category); });
  ...
  const difficulties = useMemo(() => {
    ...
    books.forEach(b => { if (typeof b.difficulty === 'number') s.add(b.difficulty); });
  ...
  const filteredBooks = useMemo(() => {
    ...
    return books.filter(b => {
      ...
      if (selectedDifficulty !== null && b.difficulty !== selectedDifficulty) return false;
      return true;
    });
          ...
          sortedBooks.length > 0 ? (
            <div className={styles.grid}>
              {sortedBooks.map((book) => {
                ...
                const progress = isActive ? Math.max(0, Math.min(100, current.progress?.percent ?? 0)) : (isFinished ? 100 : undefined);
                const status: 'in_progress' | 'finished' | undefined = isActive ? 'in_progress' : (isFinished ? 'finished' : undefined);
                return (
                <BookCard
                  key={book.id}
